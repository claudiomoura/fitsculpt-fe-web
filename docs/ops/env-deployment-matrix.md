...
| --- | --- | --- | --- |
| Web | Local | `BACKEND_URL`, `NEXT_PUBLIC_BACKEND_URL`, `NEXT_PUBLIC_API_BASE_URL` | Same-origin BFF expects backend reachable from Next runtime. |
| Web | Vercel | `BACKEND_URL`, `NEXT_PUBLIC_BACKEND_URL`, `NEXT_PUBLIC_SENTRY_DSN`, `NEXT_PUBLIC_POSTHOG_KEY`, `NEXT_PUBLIC_POSTHOG_HOST`, `NEXT_PUBLIC_APP_ENV` | `BACKEND_URL` and `NEXT_PUBLIC_BACKEND_URL` must point to the same API origin. |
...
| API | Render | `DATABASE_URL`, `DIRECT_URL`, `JWT_SECRET`, `COOKIE_SECRET`, `CORS_ORIGIN`, `APP_BASE_URL`, `OPENAI_API_KEY`, `STRIPE_SECRET_KEY`, `STRIPE_WEBHOOK_SECRET`, `STRIPE_PRO_PRICE_ID` | Use `npm run db:deploy`; never run `prisma migrate dev` on Render. |
