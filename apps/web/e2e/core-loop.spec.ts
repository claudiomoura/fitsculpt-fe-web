...
test.describe('Core loop (demo anti-regression)', () => {
  test('login → Hoy → check-in CTA opens real flow without fake writes', async ({ page }, testInfo) => {
    ...
    try {
      ...
      await expect(todayPage).toBeVisible({ timeout: 10_000 });
      ...
      await page.waitForURL(/\/app\/seguimiento\/check-in$/, { timeout: 10_000 });
      ...
      expect(afterReloadCount).toBe(beforeCount);
    } finally {
