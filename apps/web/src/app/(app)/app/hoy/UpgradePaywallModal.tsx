    ...
    <Modal open={open} onClose={onClose} title={t("today.paywallTitle")} className="today-premium-modal">
      <div className="grid gap-4" data-testid="upgrade-paywall-modal">
        <ul className="m-0 grid gap-2 p-0">
          {bullets.map((bullet) => (
            <li key={bullet} className="today-premium-modal-item list-none rounded-2xl border px-3 py-2 text-sm">
              • {bullet}
            </li>
          ))}
        </ul>
        ...
        <div>
          <p className="today-premium-modal-kicker m-0 text-xs font-semibold uppercase tracking-[0.1em]">{t("today.paywallPlansLabel")}</p>
          <p className="muted mt-2 text-sm">{PLAN_TIERS.join(" · ")}</p>
        </div>
