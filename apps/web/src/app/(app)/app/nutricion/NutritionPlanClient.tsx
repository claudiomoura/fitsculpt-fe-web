...
import QuickLogHub, { type QuickLogHubHandle } from "@/components/quick-log/QuickLogHub";
...
export default function NutritionPlanClient({
  ...
  const nutritionQuickLogRef = useRef<HTMLDivElement | null>(null);
  ...
  const planDetailsRef = useRef<HTMLElement | null>(null);
              ...
              {!loading && !error ? (
                <>
                  ...
                  <div className={styles.overlayPrimaryStack}>
                    ...
                    >
                      <div
                        ref={nutritionQuickLogRef}
                        className="mb-4 flex flex-wrap items-center justify-between gap-3 rounded-2xl border border-[var(--surface-border-default)] bg-[var(--surface-inset-bg)] px-4 py-3"
                      >
                        <div>
                          <p className="m-0 text-sm font-semibold text-[var(--text)]">
                            {safeT("nutrition.quickLogEntryTitle", "Registro rapido")}
                          </p>
                          <p className="m-0 text-xs text-[var(--muted)]">
                            {safeT("nutrition.quickLogEntryDescription", "Registra una comida con foto para el dia seleccionado")}
                          </p>
                        </div>
                        <Button
                          type="button"
                          variant="secondary"
                          onClick={() => quickLogHubRef.current?.open("meal")}
                        >
                          {t("quickLog.launcher")}
                        </Button>
                        <QuickLogHub
                          ...
                          origin="nutrition"
                          latestCheckin={null}
                          defaultMealDate={highlightedDayKey}
                          showLauncher={false}
                          onSaved={(payload) => {
                            if (payload.mode !== "meal" || !payload.date) return;
                            const parsed = parseDate(payload.date);
                            if (parsed) {
                              setSelectedDate(parsed);
                            }
                            updateNutritionSearchParams(payload.date);
                          }}
                        />
                      </div>
      ...
      >
        {selectedMealDetails ? (
          <div className="stack-md">
            {/* Header with meal type and favorite */}
            <div>
            ...
            {/* Recipe Image - larger like library */}
            ...
            {/* Description - use full recipe description if available */}
            ...
            {/* Ingredients - use full recipe ingredients if available */}
