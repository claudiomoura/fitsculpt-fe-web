...
const getMealKey = (meal: NutritionMeal, dayKey: string, index: number) => {
  const title = meal.title?.trim();
  const parts = [meal.type, title].filter(Boolean);
  return parts.length > 0 ? `${parts.join(":")}:${dayKey}` : `meal:${dayKey}:${index}`;
};
...
export default function NutritionPlanClient({ mode = "suggested" }: NutritionPlanClientProps) {
  ...
  const openMealDetail = (meal: NutritionMeal, dayKey: string, dayLabel?: string | null) => {
    setSelectedMeal({ meal, dayKey, dayLabel });
  };
                        ...
                        <div key={day.dayLabel} className="feature-card">
                          ...
                          <div className="list-grid mt-8">
                            {day.meals.map((meal, mealIndex) => (
                              <MealCard
                                key={getMealKey(meal, day.dayLabel, mealIndex)}
                                ...
                                onClick={() => openMealDetail(meal, day.dayLabel, day.dayLabel)}
                              />
                            ))}
                          </div>
                        </div>
                        ...
                        {selectedPlanDay ? (
                          <div className="list-grid">
                            {selectedPlanDay.day.meals.map((meal, mealIndex) => (
                              <MealCard
                                key={getMealKey(meal, toDateKey(selectedPlanDay.date), mealIndex)}
                                title={getMealTitle(meal, t)}
                                description={getMealDescription(meal)}
                                meta={[
                                  getMealTypeLabel(meal, t),
                                ...
                                imageUrl={getMealMediaUrl(meal)}
                                onClick={() =>
                                  openMealDetail(meal, toDateKey(selectedPlanDay.date), selectedPlanDay.day.dayLabel)
                                }
                              />
                            ))}
                          </div>
