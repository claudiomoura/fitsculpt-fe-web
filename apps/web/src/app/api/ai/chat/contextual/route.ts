...
import {
  aiRequestFailedResponse,
  mapAiUpstreamError,
  parseJsonOrNull,
} from "@/app/api/_utils/aiErrorMapping";
...
export async function POST(request: Request) {
  ...
  if (!authCookie) {
    return NextResponse.json(
      { error: "UNAUTHORIZED_NO_FS_TOKEN", debug },
      { status: 401 },
    );
  }
  ...
  try {
    ...
    const response = await fetch(`${getBackendUrl()}/ai/chat/contextual`, {
      method: "POST",
      headers: {
