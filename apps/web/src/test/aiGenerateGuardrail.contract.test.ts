...
describe("AI generate proxy guardrail contract", () => {
  ...
  it("nutrition-plan/generate preserves 4xx passthrough", async () => {
    const fetchMock = vi.fn().mockResolvedValue(mockBackendResponse(422, JSON.stringify({ error: "INVALID_INPUT" })));
    vi.stubGlobal("fetch", fetchMock);
